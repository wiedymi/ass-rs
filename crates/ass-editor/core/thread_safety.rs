...
use crate::commands::{EditorCommand, CommandResult};
...
impl SyncDocument {
    ...
    pub fn read(&self) -> Result<RwLockReadGuard<'_, EditorDocument>> {
        self.inner.read().map_err(|_| EditorError::ThreadSafetyError {
    ...
    pub fn write(&self) -> Result<RwLockWriteGuard<'_, EditorDocument>> {
        self.inner.write().map_err(|_| EditorError::ThreadSafetyError {
    ...
    pub fn execute_command<C: EditorCommand>(&self, command: C) -> Result<CommandResult> {
        ...
        let _guard = self.command_lock.lock().map_err(|_| EditorError::ThreadSafetyError {
...
impl DocumentPool {
    ...
    pub fn add_document(&self, document: EditorDocument) -> Result<String> {
        ...
        let mut docs = self.documents.write().map_err(|_| EditorError::ThreadSafetyError {
    ...
    pub fn get_document(&self, id: &str) -> Result<SyncDocument> {
        let docs = self.documents.read().map_err(|_| EditorError::ThreadSafetyError {
