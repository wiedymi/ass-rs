...
"#,
    ...
    for i in 0..200 {
        ...
        let end_time = format!("0:{:02}:{:02}.{:02}", (i + 3) / 60, (i + 3) % 60, ((i + 3) * 17) % 100);
        let style = if i % 10 == 0 { "Title" } else if i % 7 == 0 { "Sign" } else { "Default" };
...
fn test_single_edit_under_1ms() {
    ...
    let _ = doc.edit_incremental(Range::new(warm_up_pos, warm_up_pos), "X").unwrap();
    ...
    assert!(elapsed.as_micros() < 1000, "Single edit took {}µs, expected <1000µs (1ms)", elapsed.as_micros());
}
...
fn test_word_replacement_under_1ms() {
    ...
    assert!(elapsed.as_micros() < 1000, "Word replacement took {}µs, expected <1000µs (1ms)", elapsed.as_micros());
}
...
fn test_line_insertion_under_1ms() {
    ...
    assert!(elapsed.as_micros() < 1000, "Line insertion took {}µs, expected <1000µs (1ms)", elapsed.as_micros());
}
...
fn test_full_reparse_under_5ms() {
    ...
    assert!(elapsed.as_micros() < 5000, "Full reparse took {}µs, expected <5000µs (5ms)", elapsed.as_micros());
}
...
fn test_multiple_rapid_edits() {
    ...
    assert!(avg_per_edit < 1500, "Average edit time {avg_per_edit}µs, expected <1500µs (1.5ms) for rapid consecutive edits");
}
...
fn test_safe_edit_fallback_performance() {
    ...
    assert!(elapsed.as_micros() < 5000, "Safe edit took {}µs, expected <5000µs (5ms) including fallback logic", elapsed.as_micros());
}