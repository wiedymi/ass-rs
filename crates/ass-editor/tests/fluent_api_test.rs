...
fn test_fluent_line_operations() {
    ...
    doc.at_pos(pos)
        .replace_to_line_end("Modified Line 2")
        .unwrap();
...
fn test_position_builder_advanced() {
    ...
    // Build at line end
    let pos = PositionBuilder::new().at_line_end(1).build(&rope).unwrap();
    assert_eq!(pos.offset, 10); // Before newline

    // Build at document start
    let pos = PositionBuilder::at_start().build(&rope).unwrap();
