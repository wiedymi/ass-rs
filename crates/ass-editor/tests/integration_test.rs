...
"#;
    ...
    assert_eq!(doc.info().get_title().unwrap(), Some("Original Title".to_string()));
    assert_eq!(doc.info().get("ScriptType").unwrap(), Some("v4.00+".to_string()));
    
    // Test updating existing property
    doc.info().title("New Title").unwrap();
    assert_eq!(doc.info().get_title().unwrap(), Some("New Title".to_string()));
...
"#;
    ...
    doc.graphics().add_binary("subtitle_logo.png", logo_data).unwrap();
...
";
    ...
    doc.graphics().add_binary("test.png", b"image data").unwrap();
    ...
    assert_eq!(restored_doc.info().get_title().unwrap(), Some("Persistence Test".to_string()));
    assert_eq!(restored_doc.info().get("Custom Field").unwrap(), Some("Custom Value".to_string()));
    
    assert_eq!(restored_doc.fonts().list().unwrap().len(), 1);
    assert!(restored_doc.fonts().list().unwrap().contains(&"test.ttf".to_string()));
    
    assert_eq!(restored_doc.graphics().list().unwrap().len(), 1);
    assert!(restored_doc.graphics().list().unwrap().contains(&"test.png".to_string()));
