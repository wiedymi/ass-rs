...
use ass_editor::{EditorDocument, EventFilter, EventSortCriteria, EventSortOptions, EventType};
...
fn demo_filtering_operations() {
    ...
    let greeting_events = doc
        ...
        .unwrap();
    println!(
        "\nEvents containing 'hello' (case insensitive): {}",
        greeting_events.len()
    );
...
fn demo_sorting_operations() {
    ...
    for event in &by_style_then_time {
        println!(
            "  {} ({}): '{}'",
            event.event.style, event.event.start, event.event.text
        );
    }
}
...
fn demo_complex_queries() {
    ...
    for event in &filtered_events {
        println!(
            "  {} by {}: '{}'",
            event.event.start, event.event.name, event.event.text
        );
    }

    // Get first matching event
    let first_long_text = doc
        .events()
        .query()
    ...
    if let Some(event) = first_long_text {
        println!(
            "\nFirst event containing 'conversation': '{}'",
            event.event.text
        );
    }

    // Count matching events
    let narrator_count = doc
        .events()
        .query()
...
fn demo_event_properties_access() {
    ...
    println!(
        "  Event Type: {:?}",
        doc.events().event(0).event_type().unwrap()
    );
...
fn demo_main_requested_features() {
    ...
    for event in &filtered_sorted {
        println!(
            "    {} - '{}' by {}",
            event.event.start, event.event.text, event.event.name
        );
    }
    ...
    let dialogues = doc.events().dialogues().execute().unwrap();
    println!(
        "  doc.events().dialogues() found {} events",
        dialogues.len()
    );

    let by_time = doc.events().by_time().execute().unwrap();
    println!(
        "  doc.events().by_time() sorted {} events chronologically",
        by_time.len()
    );

    let with_alice = doc.events().containing("Alice").execute().unwrap();
    println!(
        "  doc.events().containing('Alice') found {} events",
        with_alice.len()
    );
